...
为了确保客户端能够刷新组策略，还需要手动更新`\\<domain.com>\Policies\<gpo id>\GPT.ini`中的`Version`，改成一个更大的数值就好
...
为了确保客户端能够刷新组策略，需要手动更新`\\<domain.com>\Policies\<gpo id>\GPT.ini`中的`Version`，改成一个更大的数值
