...
注册表键值名称经过特殊构造： 以"\0"作为开头，后面加上任意字符

对于Win32 API，"\0"(即0x00)会被识别为字符串的结束符，所以在对该字符串读取的过程中，遇到开头的"\0"，会被解析成结束符，提前截断，导致读取错误

而如果使用Native API，默认以Unicode格式，"\0"转换成unicode格式为0x0000，并不会被解析成结束符
